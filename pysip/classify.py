...
def find_best_RF_max_depth(X, Y, init_md = 20, init_ne = 100, n_jobs = 15, n_splits = 5, verbose = True, use_kappa = False):
    ...
    if verbose: 
        ...
        if use_kappa: print('    Using Kappa as metric')
    ...
    for j, (train_index, test_index) in enumerate(skf.split(X, Y)):
        if verbose: print(f'fold {j+1} de {n_splits}.',end='\n')
        X_train, X_test = X[train_index], X[test_index]
        y_train, y_test = Y[train_index], Y[test_index]
        if verbose: 
            it = tqdm(enumerate(lmd))
        else: 
            it = enumerate(lmd)
        for i,md in it:
            ...
            accTest[i,j] = metric(y_test, clf.predict(X_test))
            accTrain[i,j] = metric(y_train, clf.predict(X_train))
    ...
    if verbose: print(f'El mejor resultado en el conjunto de testeo fue obtenido para un arbol de profundidad {best_prof} y una exactitud de {best_acc:.3f}')
    imx = np.where(scoresTest>best_acc-0.002)[0][0]
    best_prof = lmd[imx]
    best_acc = scoresTest[imx]
    if verbose: print(f'Me quedo con una profundidad de {best_prof} y una exactitud de {best_acc:.3f} (ne={init_ne}).')
...
def find_best_RF_n_estimators(X, Y, init_md = 20, init_ne = 100, n_jobs = 15, n_splits = 5, verbose = True, use_kappa = False):
    ...
    for j, (train_index, test_index) in enumerate(skf.split(X, Y)):
        ...
        for i,ne in it:
            ...
            #clf = tree.DecisionTreeClassifier(max_depth = md)
            clf = clf.fit(X_train, y_train)
            accTest_ne[i,j] = metric(y_test, clf.predict(X_test))
            accTrain_ne[i,j] = metric(y_train, clf.predict(X_train))
    #%
    if verbose: print()
    #Calculo promedios de los K folds
    ...
    best_ne = lne[imx]
    best_acc = scoresTest[imx]
    if verbose: print(f'El mejor resultado en el conjunto de testeo fue obtenido para un n_estimators {best_ne} y una exactitud de {best_acc:.3f}')
    imx = np.where(scoresTest>best_acc-0.002)[0][0]
    best_ne = lne[imx]
    best_acc = scoresTest[imx]
    if verbose: print(f'Me quedo con un n_estimators {best_ne} y una exactitud de {best_acc:.3f} (md={init_md}).')
    #%
    #Grafico promedios
    plt.figure()
